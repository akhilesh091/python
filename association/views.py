...
class EditCourtView(APIView):
    def patch(self, request, id):
        try:
            court=Court.objects.get(id=id)
            serializer = CourtListSerializer(court, data=request.data)
            if serializer.is_valid():
                serializer.save()
...
class MembershipPlanView(APIView):
    def post(self ,request):
        ...
        if serializer.is_valid():
            ...
            if MembershipPlan.objects.filter(duration = duration ,unit_of_plan = unit).exists() :
...
class ToggleMembershipPlanView(APIView):
    ...
    def get(self, request, id) :
        ...
        serializer = MembershipPlanSerializer(data)
        return Response(serializer.data ,status=status.HTTP_200_OK)
    ...
    def patch(self, request, id):
        ...
        try:
            ...
            serializer=MembershipPlanSerializer(plan, data)
...
class ToggleMembershipFineAmountView(APIView):
    ...
    def patch(self, request, id):
        ...
        try:
            ...
            serializer=MembershipFineAmountSerializer(fine, data)
            if serializer.is_valid():
                serializer.save()
